...
import { STATUS_COLOR, STATUS_LABEL, colorHex } from "@/lib/colors";
import { parseSearchOutput, type Candidate } from "@/lib/parseSearch";
...
const EXPLORER_URL = "https://amoy.polygonscan.com/address/0x6650630946313835E5a1175ddE8F9969674fbE28";
...
export default function RecordingDemo() {
  ...
  const candidates = useMemo(() => parseSearchOutput(outputOf(fullResult)), [fullResult]);
  ...
  return <main className="mx-auto min-h-screen max-w-5xl px-5 py-10 text-zinc-900 sm:px-8">
    <header className="rounded-3xl bg-zinc-950 px-7 py-9 text-white shadow-xl sm:px-10">
      ...
      <a href={EXPLORER_URL} target="_blank" rel="noreferrer" className="mt-5 inline-flex rounded-lg border border-violet-300/50 bg-violet-400/10 px-3 py-2 font-mono text-xs text-violet-100 underline underline-offset-2 hover:bg-violet-400/20">Open EvidenceRegistry in Amoy Explorer ↗</a>
    </header>
    ...
    <section className="mt-6 grid gap-5 lg:grid-cols-2">
      <Card number="1" title="Explicit owner enrollment" tone="emerald"><p className="text-sm text-zinc-600">Enroll any clear photo for a person who has explicitly consented. Add known public domains to scope discovery, or leave domains empty for the broad public-web policy.</p><label className="mt-4 block text-sm font-medium">Enrollment photo<input className="mt-1 block w-full text-sm" type="file" accept="image/*" onChange={(e) => setEnrollmentPhoto(e.target.files?.[0] ?? null)} /></label><label className="mt-3 block text-sm font-medium">Subject ID<input placeholder="e.g. alex-demo" className="mt-1 w-full rounded-lg border border-zinc-300 px-3 py-2" value={subject} onChange={(e) => setSubject(e.target.value)} /></label><label className="mt-3 block text-sm font-medium">Approved domain(s) <span className="font-normal text-zinc-500">optional, comma-separated</span><input placeholder="e.g. linkedin.com,github.com" className="mt-1 w-full rounded-lg border border-zinc-300 px-3 py-2 font-mono text-sm" value={domains} onChange={(e) => setDomains(e.target.value)} /></label><label className="mt-4 flex gap-2 text-sm"><input type="checkbox" checked={consent} onChange={(e) => setConsent(e.target.checked)} />I confirm this owner explicitly consents to enrollment and scoped search.</label><button className={`${primary} mt-4`} onClick={enroll} disabled={busy !== null || !enrollmentPhoto || !subject.trim() || !consent}>{busy === "enroll" ? "Encoding face…" : "Enroll with consent"}</button><Result label="Enrollment" data={enrollResult} success="enrolled" /></Card>
      <Card number="2" title="Face scan input" tone="blue"><p className="text-sm text-zinc-600">Upload any face image of the consented owner to detect and encode. A web match is shown only when the image, or a visually matching image, exists on a public indexed page.</p><label className="mt-4 block text-sm font-medium">Proof image<input className="mt-1 block w-full text-sm" type="file" accept="image/*" onChange={(e) => setScanPhoto(e.target.files?.[0] ?? null)} /></label><button className={`${primary} mt-4`} onClick={scan} disabled={busy !== null || !scanPhoto || !enrolled}>{busy === "scan" ? "Detecting and encoding…" : "Detect and encode face"}</button><Result label="Face scan" data={scanResult} success="status       : ok" /></Card>
    </section>
    <section className="mt-6 rounded-2xl border border-violet-200 bg-gradient-to-br from-violet-50 to-white p-6 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-mono text-xs font-bold uppercase tracking-[0.16em] text-violet-700">3 · Full verification pipeline</p><h2 className="mt-1 text-2xl font-bold">Consent → web match → Amoy proof</h2><p className="mt-1 text-sm text-zinc-600">Google Vision searches the consented domain, InsightFace verifies returned faces, then canonical evidence is anchored and re-checked on Polygon Amoy.</p></div><button className={primary} onClick={runPipeline} disabled={busy !== null || !scanPhoto || !enrolled || !scanResult?.ok}>{busy === "full" ? "Running live pipeline…" : "Run live verification"}</button></div>{fullResult && <><div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{milestones.map((item) => <div key={item.label} className={`rounded-xl border p-4 ${item.ok ? "border-emerald-200 bg-emerald-50" : "border-red-200 bg-red-50"}`}><p className="text-sm font-bold">{item.ok ? "✓" : "!"} {item.label}</p><p className="mt-2 break-all font-mono text-xs text-zinc-700">{item.detail}</p></div>)}</div><CandidateResults candidates={candidates} /></>}<Result label="Full pipeline audit trail" data={fullResult} success="INTEGRITY CONFIRMED" open /></section>
...
function CandidateResults({ candidates }: { candidates: Candidate[] }) { if (!candidates.length) return <p className="mt-5 rounded-xl border border-zinc-200 bg-white p-4 text-sm text-zinc-600">No fetchable candidate URLs were returned for this photo. The consent gate can still pass; web verification requires public indexed matching content.</p>; const verified = candidates.filter((c) => c.status === "verified"); const rejected = candidates.filter((c) => c.status !== "verified"); const row = (candidate: Candidate, index: number) => <li key={`${candidate.url}-${index}`} className="border-t border-zinc-100 px-4 py-3 first:border-t-0"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full px-2 py-0.5 text-xs font-bold" style={{ backgroundColor: `${colorHex(STATUS_COLOR[candidate.status])}18`, color: colorHex(STATUS_COLOR[candidate.status]) }}>{STATUS_LABEL[candidate.status]}</span>{candidate.similarity !== undefined && <span className="font-mono text-xs text-zinc-600">similarity {candidate.similarity.toFixed(4)}</span>}<span className="font-mono text-xs text-zinc-500">{candidate.host}</span></div><a href={candidate.url} target="_blank" rel="noreferrer" className="mt-1 block break-all font-mono text-xs text-blue-700 underline decoration-blue-300 underline-offset-2 hover:text-blue-900">{candidate.url} ↗</a>{candidate.reason && <p className="mt-1 text-xs text-zinc-500">{candidate.reason}</p>}</li>; return <div className="mt-6 grid gap-4 lg:grid-cols-2"><section className="overflow-hidden rounded-xl border border-emerald-200 bg-white"><div className="bg-emerald-50 px-4 py-3 text-sm font-bold text-emerald-800">Verified public matches ({verified.length})</div><ul>{verified.length ? verified.map(row) : <li className="p-4 text-sm text-zinc-500">No verified match.</li>}</ul></section><section className="overflow-hidden rounded-xl border border-zinc-200 bg-white"><div className="bg-zinc-50 px-4 py-3 text-sm font-bold text-zinc-700">Rejected / unavailable candidates ({rejected.length})</div><ul>{rejected.length ? rejected.map(row) : <li className="p-4 text-sm text-zinc-500">No rejected candidates.</li>}</ul></section></div>; }
